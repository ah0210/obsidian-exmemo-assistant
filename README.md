...
* 新增 Slug 生成功能：根据文章内容生成英文 SEO slug，并写入 slug 字段。
* 新增 Collections 自动匹配：从候选合集列表中匹配正文内容，支持字符串匹配和 LLM 智能匹配两种模式，写入 collections 字段。
* 提示词完全自定义：移除所有固定追加内容，完全使用用户自定义提示词，支持更灵活的控制。
* 合集 LLM 匹配：新增 LLM 智能匹配合集功能，可自定义提示词，结果自动过滤。
...
1. 构建完成后，将整个项目文件夹复制到 Obsidian 的插件目录
2. 或仅复制 `dist/` 目录下的文件到插件文件夹
