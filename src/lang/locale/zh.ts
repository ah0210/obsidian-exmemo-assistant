...
export default {
    ...
    "baseUrl": "基础 URL",
    "modelName": "模型名称",
    "modelNameDesc": "可填写多个，默认使用第一项（可用上移调整），第一个失败会尝试后续模型",
    "addModel": "添加模型",
    "defaultModel": "默认",
    "candidateModel": "候选",
    "llmMaxRetries": "最大重试次数",
    "llmMaxRetriesDesc": "LLM 调用失败时的最大重试次数",
    "llmTimeout": "请求超时时间（毫秒）",
    "llmTimeoutDesc": "LLM API 请求的超时时间",
    "llmTemperature": "温度",
    "llmTemperatureDesc": "LLM 输出的随机性，0-2 之间，越高越随机",
    "llmMaxTokens": "最大输出 Token 数",
    "llmMaxTokensDesc": "LLM 单次响应的最大 Token 数",
    ...
    "enableContentOptimizeDesc": "启用后，会先生成元数据，然后根据提示词优化文章内容",
    "contentOptimizePrompt": "文章优化提示词",
    "contentOptimizePromptDesc": "用于优化文章内容的提示词",
    "defaultContentOptimizePrompt": "请优化这篇文章的内容，使其更加通顺、清晰、易读，保持原意不变。只返回优化后的文章内容，不要添加其他说明。",
    "contentOptimized": "文章内容已优化",

    "extractCover": "提取封面图",
    "extractCoverDesc": "从文章内容中提取第一张图片作为封面图",
    "enableExtractCover": "启用提取封面图",
    "enableExtractCoverDesc": "启用后会自动提取文章中的第一张图片作为封面图",
